...
def anagram(s):
    ...
    if len(s) % 2 != 0:
        return -1  # If the length of the string is not even, it's not possible to split it into two equal parts.

    half_length = len(s) // 2
    s1 = s[:half_length]
    s2 = s[half_length:]

    # Count occurrences of each character in both substrings
    count_s1 = [0] * 26
    count_s2 = [0] * 26
    ...
    diff = 0
    for i in range(26):
        diff += abs(count_s1[i] - count_s2[i])

    # Return the number of operations needed to make the two substrings anagrams
    return diff // 2
